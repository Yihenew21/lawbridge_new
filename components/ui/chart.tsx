    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          ...
          const indicatorColor = color || (payloadItem.payload as Record<string, unknown>)?.fill as string || payloadItem.color as string

          return (
            <div
              key={payloadItem.dataKey as string}
              className={cn(
            ...
            >
              {formatter && payloadItem?.value !== undefined && payloadItem.name ? (
                formatter(payloadItem.value, payloadItem.name, payloadItem, index, payloadItem.payload)
              ) : (
                <>
                  {itemConfig?.icon ? (
                    <itemConfig.icon />
                  ...
                  >
                    <div className="grid gap-1.5">
                      ...
                      <span className="text-muted-foreground">
                        {itemConfig?.label || payloadItem.name as string}
                      </span>
                    </div>
                    {payloadItem.value != null && (
                      <span className="text-foreground font-mono font-medium tabular-nums">
                        {String((payloadItem.value as number).toLocaleString())}
                      </span>
                    )}
                  </div>
                </>
